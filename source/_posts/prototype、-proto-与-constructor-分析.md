...
函数也是对象，较为特殊的是，除了 `__proto__` 和 constructor，**函数** 还有 prototype 这个独有属性。prototype 是显式原型，`__proto__` 是隐式原型。
...
对象的 `__proto__` 属性是创建对象时自动添加的，默认值为其构造函数的 prototype。函数的 prototype 属性是定义时自动添加的，默认为 { }，一层一层的 `__proto__` 便形成了 **原型链**。
